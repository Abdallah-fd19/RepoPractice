  ...
  return (
    <div className="flex h-screen bg-[var(--color-bg)] overflow-hidden">
      ...
      <main className="flex-1 p-6 lg:p-8 overflow-y-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-[var(--color-text)]">Your Repos</h1>
        ...
        {!loading && repos.length === 0 && (
          <div className="mb-6 bg-[var(--color-warning-bg)] border border-[var(--color-warning-border)] rounded-lg p-4">
            <p className="text-sm text-[var(--color-warning-text)] font-medium">
              No GitHub account connected.{" "}
              <a href={`${API}/auth/github/login/`} className="underline hover:brightness-110">
                Connect GitHub
              </a>{" "}
              to load your repos.
            </p>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-36 rounded-xl shimmer-loading" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
